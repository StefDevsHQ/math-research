...
| `SS-005` | For the assignment-target query functions induced by even square-grid monotone two-CNF formulas, every ordered assignment-target query graph has size `2^{Omega(L^{1/4})}`, where `L` is binary query-instance length. | `PROVED` | `CHECKED` | [Grid-family ordered barrier](routes/exact-state-compression-barriers/proofs/grid-family-state-barrier.md) | 2026-07-16 |
...
The exact-state barrier route has established a superpolynomial lower bound for one explicitly defined ordered Boolean query-representation model. It has not established that the interval, progression, residue, exception, or recursive summaries from structural compression compile into that model or into the route's explicit tree-state model. The result is not a lower bound for arbitrary fixed-target Subset Sum algorithms and is not evidence sufficient for `P != NP`.
